...
async fn test_minimal_agent() {
    let result = run_program(r#"agent A { systemPrompt: "x" model: "gpt-4o-mini" } let r = A.run()"#).await;
    assert!(result.is_ok());
}
...
async fn test_run_with_string_input() {
    let result = run_program(
        ...
        let result = Echo.userPrompt("Hello there").run()
    "#,
    )
    .await;
    assert!(result.is_ok());
...
async fn test_whitespace_variations() {
    let result =
        run_program("agent   A   {   systemPrompt  :   \"x\"   model: \"gpt-4o-mini\"  }   let r = A.run()")
            .await;
    assert!(result.is_ok());
}
...
async fn test_error_undefined_agent() {
    expect_failure("let r = Ghost.run()", "Undefined").await;
}
...
async fn test_error_missing_model() {
    ...
    expect_failure(
        r#"agent NoModel { systemPrompt: "Hello" } let r = NoModel.run()"#,
        "model",
    )
    .await;
}
...
async fn test_error_run_before_define() {
    expect_failure(
        r#"let r = Later.run() agent Later { systemPrompt: "x" model: "gpt-4o-mini" }"#,
        "Undefined",
    )
    .await;
}
...
async fn test_agent_with_empty_body() {
    ...
    expect_failure("agent Empty { } let r = Empty.run()", "missing").await;
}
...
async fn test_long_prompt() {
    ...
    let source = format!(
        r#"agent Long {{ systemPrompt: "{}" model: "gpt-4o-mini" }} let r = Long.run()"#,
        long_prompt
    );
    let result = run_program(&source).await;
    assert!(result.is_ok());
